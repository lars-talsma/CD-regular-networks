"""
Skeleton of the simulations of a quantum network with an infinite honeycomb
...
def simulation_infinite_honeycomb(prob_succ_gen, prob_succ_swap, prob_swap,
    ...
    an infinite honeycomb grid of nodes (which is represented by a finite
    honeycomb lattice containing 'number_of_nodes' × 'number_of_nodes'
    ...
    virtual_link_adj_array = np.empty((number_of_nodes, 2*number_of_nodes,
                                       number_of_nodes, 2*number_of_nodes,
                                       qubits_per_node))
    virtual_link_adj_array[:] = np.inf

    swap_dist_adj_array = np.zeros((number_of_nodes, 2*number_of_nodes,
                                    number_of_nodes, 2*number_of_nodes,
                                    qubits_per_node))
    qubit_orient_adj_array = np.zeros((number_of_nodes, 2*number_of_nodes,
                                       number_of_nodes, 2*number_of_nodes,
                                       qubits_per_node))

    virtual_node_degree = np.empty((number_of_nodes, 2*number_of_nodes, time))
    virtual_neighbourhood_size = np.empty((number_of_nodes, 2*number_of_nodes,
                                           time))
    ...
    for t in range(time):
        ...
        np.testing.assert_array_less(
            ...
            3*min([qubits_per_node, time_cutoff]) + 1)
        np.testing.assert_array_less(
            virtual_neighbourhood_size[:, :, t],
            min([3*qubits_per_node,
                 3/2*(max_swap_dist + 1)*(max_swap_dist + 2),
                 3*time_cutoff]) + 1
        )
        np.testing.assert_array_less(
            virtual_neighbourhood_size[:, :, t],
...
def entanglement_generation(prob_succ_gen, virtual_link_adj_array,
    ...
    cart_prod_node_indices = product(np.arange(0, number_of_nodes),
                                     np.arange(0, 2*number_of_nodes, 4))
    ...
    for node_index_hor, node_index_vert in cart_prod_node_indices:
        ...
        if prob_succ_gen >= np.random.random():
            free_qubit = smallest_free_qubit(
                node_index_hor, node_index_vert,
                node_index_hor, (node_index_vert - 1) % (2*number_of_nodes),
                virtual_link_adj_array)
    ...
    cart_prod_node_indices = product(np.arange(0, number_of_nodes),
                                     np.arange(2, 2*number_of_nodes, 4))

    for node_index_hor, node_index_vert in cart_prod_node_indices:
        if prob_succ_gen >= np.random.random():
            free_qubit = smallest_free_qubit(
                node_index_hor, node_index_vert,
                (node_index_hor + 1) % number_of_nodes, node_index_vert + 1,
                virtual_link_adj_array)
...
def entanglement_swap(prob_succ_swap, prob_swap, time_cutoff, max_swap_dist,
    ...
    link_within_node_adj_array = np.empty((number_of_nodes, 2*number_of_nodes,
                                           number_of_nodes, 2*number_of_nodes,
                                           qubits_per_node))
...
def virtual_metrics(virtual_link_adj_array):
    ...
    cart_prod_node_indices_array = (
        np.array(np.meshgrid(
            ...
            np.arange(0, 2*number_of_nodes))).T.reshape(-1, 2))

    virtual_node_degree = np.empty((number_of_nodes, 2*number_of_nodes))
    virtual_neighbourhood_size = np.empty((number_of_nodes, 2*number_of_nodes))
