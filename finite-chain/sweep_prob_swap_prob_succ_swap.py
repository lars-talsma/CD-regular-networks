...
def run_simulation(params):
    ...
    number_of_nodes = 11 # 2*(max_swap_dist+1)+1
    time = 500000
    test = False

    prob_succ_swap, prob_swap = params
    
    np.random.seed(0)
    
    virtual_link_age_array, virtual_node_degree, virtual_neighbourhood_size, swap_count, link_fail_count = skeleton.simulation_finite_chain(prob_succ_gen, prob_succ_swap, prob_swap, time_cutoff, qubits_per_node, time, max_swap_dist, number_of_nodes, test)
