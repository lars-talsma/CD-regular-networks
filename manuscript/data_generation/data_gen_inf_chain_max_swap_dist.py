...
import numpy as np
import data_gen_inf_chain_max_swap_dist_run_sim


prob_swap_sweep_size = 100+1
prob_swap_range = np.arange(1, prob_swap_sweep_size)/(prob_swap_sweep_size-1) # Only run probabilistic swaps
...
if __name__ == '__main__':
    ...
    params = product(max_swap_dist_range, prob_swap_range)
    pool = multiprocessing.Pool(70)
    results = pool.map(data_gen_inf_chain_max_swap_dist_run_sim.run_simulation, params, chunksize=1)

end = timer()
print(end - start) # Time in seconds

np.save('data/data_inf_chain_max_swap_dist_reproduciblity_test', results)