...
def run_simulation(params):
    ...
    max_swap_dist, prob_swap = params 
    number_of_nodes = 2*(max_swap_dist+1)+1
    sim_time = 3*time_cutoff
    qubits_per_node = 3*time_cutoff  # Represents an infinite memory for an infinite square lattice
    ...
    for n in range(N_sample):
        (virtual_link_age_array, virtual_node_degree, virtual_neighbourhood_size, swap_count, link_fail_count) = (
            skeleton_inf_honeycomb.simulation_infinite_honeycomb(prob_succ_gen, prob_succ_swap, prob_swap, time_cutoff, 
    ...
    virtual_node_degree_error = (3*time_cutoff-0)/np.sqrt(N_sample)
    virtual_neighbourhood_size_error = (3*np.minimum(time_cutoff, (max_swap_dist+1)*(max_swap_dist+2)/2)-0)/np.sqrt(N_sample)
    ...
    print(max_swap_dist, prob_swap, end - start)
