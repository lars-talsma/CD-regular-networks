...
import numpy as np
import data_gen_inf_sq_prob_succ_swap_run_sim
...
prob_succ_swap = [1, 0.75, 0.5]

start = timer()

if __name__ == '__main__':
    # Take the cartesian product as the parameters because pool.map() takes only one iterable
    params = product(prob_succ_swap, prob_swap_range)
    pool = multiprocessing.Pool(38)
    results = pool.map(data_gen_inf_sq_prob_succ_swap_run_sim.run_simulation, params, chunksize=1)

end = timer()
print(end - start) # Time in seconds

np.save('data/data_inf_sq_prob_succ_swap', results)