"""
Skeleton of the simulations of a quantum network with an infinite
triangular lattice topology performing entanglement generation,
entanglement swaps and cutoffs.
"""
...
def simulation_infinite_triangular(prob_succ_gen, prob_succ_swap, prob_swap,
    ...
    an infinite triangular lattice of nodes (which is represented by a
    finite triangular grid of 'number_of_nodes' × 'number_of_nodes'
    ...
    number_of_nodes : int
        The number of nodes in the finite triangular lattice of nodes
    test: Bool
    ...
    for t in range(time):
        ...
        np.testing.assert_array_less(
            ...
            6*min([qubits_per_node, time_cutoff]) + 1)
        np.testing.assert_array_less(
            virtual_neighbourhood_size[:, :, t],
            min([6*qubits_per_node,
                 3*(max_swap_dist + 1)*(max_swap_dist + 2),
                 6*time_cutoff]) + 1
        )
        np.testing.assert_array_less(
            virtual_neighbourhood_size[:, :, t],
...
def continuous_distribution_protocol(prob_succ_gen, prob_succ_swap, prob_swap,
    ...
    infinite triangular lattice of 'number_of_nodes × number_of_nodes'
    nodes with each a qubit register containing 'qubits_per_node', where
    (1) entanglement is generated with a probability 'prob_succ_gen',
    (2) entanglement swaps are attempted with probability 'prob_swap'
    and succeed with 'prob_succ_swap' and (3) virtual links older than
...
def entanglement_generation(prob_succ_gen, virtual_link_adj_array,
    ...
    cart_prod_node_indices = product(np.arange(0, number_of_nodes, 2),
    ...
    # top-right ((node_index_hor + 1) % number_of_nodes,
    # (node_index_vert + 1) % number_of_nodes) and bottom-right
    ...
    for node_index_hor, node_index_vert in cart_prod_node_indices:
        if prob_succ_gen >= np.random.random():
            free_qubit = smallest_free_qubit(
                ...
                node_index_hor, (node_index_vert + 1) % number_of_nodes,
        ...
        if prob_succ_gen >= np.random.random():
            free_qubit = smallest_free_qubit(
                node_index_hor, node_index_vert,
                (node_index_hor + 1) % number_of_nodes,
                (node_index_vert + 1) % number_of_nodes,
                virtual_link_adj_array)
    ...
    cart_prod_node_indices = product(np.arange(1, number_of_nodes, 2),
                                     np.arange(0, number_of_nodes))
    for node_index_hor, node_index_vert in cart_prod_node_indices:
        if prob_succ_gen >= np.random.random():
            free_qubit = smallest_free_qubit(
