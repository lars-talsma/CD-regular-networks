"""
Skeleton of the simulations of a quantum network with a finite chain
topology performing entanglement generation, entanglement swaps and
cutoffs.
"""
...
def simulation_finite_chain(prob_succ_gen, prob_succ_swap, prob_swap,
    ...
    a finite lattice containing 'number_of_nodes' nodes where the nodes
    have a 'qubits_per_node'-sized qubit register. Entanglement between
    nodes is generated successfully with a probability 'prob_succ_gen',
    ...
    virtual_link_adj_array : numpy.ndarray
        number_of_nodes × 1 × number_of_nodes x 1 × qubits_per_node-
    ...
    virtual_node_degree : numpy.ndarray
        number_of_nodes × 1 × time-dimensional array storing the virtual
...
def continuous_distribution_protocol(prob_succ_gen, prob_succ_swap, prob_swap,
    ...
    finite chain of 'number_of_nodes' nodes with each a qubit register
    containing 'qubits_per_node', where (1) entanglement is generated
    with a probability 'prob_succ_gen', (2) entanglement swaps are
    attempted with probability 'prob_swap' and succeed with
    'prob_succ_swap' and (3) virtual links older than time_cutoff are
    removed. Virtual links can be at most of distance 'max_swap_dist'.
    Store the ages of the virtual links (which are characterized by
    the indices of the two nodes it connects and the qubit index storing
    the link) in an adjacency array, 'virtual_link_adj_array', the
    number of swaps required to generate the link,
    'swap_dist_adj_array'-1, and the orientation of the qubits storing a
    link, 'qubit_orient_adj_array'. Record the number of swaps attempted,
    'swap_count', and failed swaps, 'swap_fail_count'.
...
def entanglement_generation(prob_succ_gen, virtual_link_adj_array,
    ...
    # All nodes but the most right one attempt to generate entanglement with
    # the node positioned to the right ((node_index_hor + 1), node_index_vert)
    cart_prod_node_indices = product(np.arange(0, number_of_nodes-1),
                                     np.arange(0, 1))
