"""
Skeleton of the simulations of a quantum network with a finite square
...
def simulation_finite_square(prob_succ_gen, prob_succ_swap, prob_swap,
    ...
    an infinite square grid of 'number_of_nodes' × 'number_of_nodes'
    nodes where the nodes have a qubits_per_node'-sized qubit register.
    Entanglement between nodes is generated successfully with a
    probability 'prob_succ_gen', entanglement swaps happen with a
    probability 'prob_swap' and succeed with probability
    'prob_succ_swap'. Entangled pairs are cutoff after  living
    'time_cutoff' and the maximum number of swaps allowed to generate a
    virtual link is 'max_swap_dist'. Return the ages of the virtual
    links (which are characterized by the horizontal and vertical
    ...
    number_of_nodes : int
        The number of nodes in the finite square lattice of nodes
    test: Bool
    ...
    """
    # The adjacency array on virtual links contains entries equal to infinity
    # to denote that no links exist, while the arrays on the swap distance and
    # qubit orientation contain elements equal to zero
    virtual_link_adj_array = np.empty((number_of_nodes, number_of_nodes,
                                       number_of_nodes, number_of_nodes,
                                       qubits_per_node))
    virtual_link_adj_array[:] = np.inf

    swap_dist_adj_array = np.zeros((number_of_nodes, number_of_nodes,
                                    number_of_nodes, number_of_nodes,
                                    qubits_per_node))
    qubit_orient_adj_array = np.zeros((number_of_nodes, number_of_nodes,
                                       number_of_nodes, number_of_nodes,
                                       qubits_per_node))

    virtual_node_degree = np.empty((number_of_nodes, number_of_nodes, time))
    virtual_neighbourhood_size = np.empty((number_of_nodes, number_of_nodes,
                                           time))
    ...
    for t in range(time):
        ...
        np.testing.assert_array_less(
            ...
            4*min([qubits_per_node, time_cutoff]) + 1)
        np.testing.assert_array_less(
            virtual_neighbourhood_size[:, :, t],
            min([4*qubits_per_node,
                 2*max_swap_dist**2 + 6*max_swap_dist + 4,
                 4*time_cutoff]) + 1
        )
        np.testing.assert_array_less(
            virtual_neighbourhood_size[:, :, t],
...
def continuous_distribution_protocol(prob_succ_gen, prob_succ_swap, prob_swap,
                                     ...
                                     qubit_orient_adj_array):
    """
    Perform one round of a continuous distribution protocol for a
    finite square lattice of 'number_of_nodes × number_of_nodes' nodes
    ...
    time_cutoff are removed. The maximum number of swaps allowed to
    generate a virtual link is 'max_swap_dist'. Store the ages of the
    virtual links (which are characterized by the horizontal and
    vertical indices of the two nodes it connects and the qubit index
    storing the link) in an adjacency array, 'virtual_link_adj_array',
    the number of swaps required to generate the link,
    'swap_dist_adj_array'-1, and the orientation of the qubits storing a
    link, 'qubit_orient_adj_array'. Record the number of swaps
    attempted, 'swap_count', and failed swaps, 'swap_fail_count'.
...
def entanglement_generation(prob_succ_gen, virtual_link_adj_array,
                            swap_dist_adj_array, qubit_orient_adj_array):
    """
    Perform a round of entanglement generation of the CD protocol where
    all nodes attempt to generate entanglement with the node
    ...
    cart_prod_node_indices = product(np.arange(0, number_of_nodes),
                                     np.arange(0, number_of_nodes))

    # Entanglement generation is attempted by all nodes except those on
    # the top and right borders of the square with the node positioned to
    # the top (node_index_hor, node_index_vert + 1) and right
    # (node_index_hor + 1, node_index_vert)
    for node_index_hor, node_index_vert in cart_prod_node_indices:
        if prob_succ_gen >= np.random.random():
            if (node_index_vert + 1) % number_of_nodes > node_index_vert:
                free_qubit = smallest_free_qubit(
                    node_index_hor, node_index_vert,
                    node_index_hor, node_index_vert + 1,
            ...
            if prob_succ_gen >= np.random.random():
                if (node_index_hor + 1) % number_of_nodes > node_index_hor:
                    free_qubit = smallest_free_qubit(
                        node_index_hor, node_index_vert,
                        node_index_hor + 1, node_index_vert,
                        virtual_link_adj_array)
...
def entanglement_swap(prob_succ_swap, prob_swap, time_cutoff, max_swap_dist,
    ...
    link_within_node_adj_array = np.empty((number_of_nodes, number_of_nodes,
                                           number_of_nodes, number_of_nodes,
                                           qubits_per_node))
...
def smallest_free_qubit(node_index_hor_1, node_index_vert_1,
    ...
    # qubit register (note that np.where returns a (·,) tuple so we need
    # to call [0] to retrieve the qubit index).
...
def virtual_metrics(virtual_link_adj_array):
    ...
    cart_prod_node_indices_array = (
        np.array(np.meshgrid(np.arange(0, number_of_nodes),
                             np.arange(0, number_of_nodes))).T.reshape(-1, 2))

    virtual_node_degree = np.empty((number_of_nodes, number_of_nodes))
    virtual_neighbourhood_size = np.empty((number_of_nodes, number_of_nodes))
